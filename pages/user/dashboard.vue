                  ...
                  <b-row >
                    <div > 
                         ...
                         <div v-for="(item) in completedLiveExams" :key="item.id">
                            <b-card :title=item.examName img-src="https://picsum.photos/600/300/?image=25"
                                img-alt="Image" img-top style="max-width:21rem;" class="mb-2">
                                    <b-button variant="primary" @click="goToResultPage(item)">Go To Result</b-button>
                            </b-card> 
                        </div>    
                         </b-card-group> 
                     </div>
...
export default {
  name: 'Dashboard',
    ...
    getCompeletedLiveExam(userId) {
         this.$axios.get("/liveexaminfo/compeleted-live-exam/0/10",
               {headers: {user:userId}}).then(response => {
